...
{
    //记录未执行事件流
        private readonly Queue<IEventInfo> _delayActions = new Queue<IEventInfo>();
        //记录执行协程的事件流
        private readonly List<Coroutine> _ies = new List<Coroutine>();
        ...
        public void InvokeDelayActions()
        {
            ...
            while (_delayActions.Count != 0)
            {
                ...
                _ies.Add(MonoMgr.GetInstance().StartCoroutine(DelayToCallEnumerator(currentDelayEvent.actions, currentDelayTime)));
            }
            ClearDelayActions();
        }
        public void InvokeDelayActions<T>(T[] info)
        {
            ...
            while (_delayActions.Count != 0)
            {
                ...
                currentDelayTime += currentDelayEvent.DelayTime;
                _ies.Add(MonoMgr.GetInstance().StartCoroutine(DelayToCallEnumerator(currentDelayEvent.actions, info[currentIndex], currentDelayTime)));
                currentIndex++;
            }
            ClearDelayActions();
        }
        ...
        private void StopDelayEnumerator()
        {
            ...
            {
                MonoMgr.GetInstance().StopCoroutine(e);
            }
            _ies.Clear();
        }
        public void Dispose()
